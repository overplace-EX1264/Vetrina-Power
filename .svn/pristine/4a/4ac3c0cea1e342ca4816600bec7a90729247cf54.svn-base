...
.run(function($ionicPlatform, $cordovaPush, Database){
	...
	window.database = {
    	...
    	addQueue: function (q, c){
    		...
    		if (typeof q == 'object' && (!q.hasOwnProperty('query') || !q.hasOwnProperty('params') || typeof q.query != 'string' || typeof q.params != 'object')){ return false; }
    		...
    		$cordovaPush.register(config)
    		    .then(function(result){
                    ...
                    if(ionic.Platform.isIOS()){
                        ...
                        if (!window.database.isOpen){
                        	window.database.addQueue({query: "INSERT INTO auth SET id = 1, regid = ? ON DUPLICATE KEY UPDATE auth SET regid = ?;", params: [regId, regId]}, function (tx, res){
                        		
                        		
                        	});
                        }else {
                        	window.database.mSQLite.transaction(function (tx){
                        		tx.executeSql("INSERT INTO auth SET id = 1, regid = ? ON DUPLICATE KEY UPDATE auth SET regid = ?;", [regId, regId], function (tx, res){
...
.controller('LoginCtrl', function($scope, $rootScope, $state, $ionicLoading, $cordovaNetwork, $cordovaToast, Auth, Database, Convalide, Chats, NavBadges) {
    ...
    var query = "SELECT id, id_scheda, regid, app_type, app_version, username, timestamp FROM auth WHERE username IS NOT NULL LIMIT 0,1;";
                    ...
                    if (!window.database.isReady){
                    	window.database.addQueue(query, function (tx, res){
                    		if ($scope.credentials.remember){
                    			...
                    			query = "REPLACE INTO auth (id, id_scheda, regid, app_type, app_version, username, timestamp) VALUES (1, ?, NULL, ?, ?, ?, ?) ";
                    			tx.executeSql(query, [response.data.id_scheda, "myOverplace", "2", $scope.credentials.username, timestamp], function (){
                        			$ionicLoading.hide();
